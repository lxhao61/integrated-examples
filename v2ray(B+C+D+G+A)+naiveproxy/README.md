...
1、B=vless+ws+tls（tls由caddy提供及处理，不需配置；另可添加或更改为其它WS类应用，参考服务端单一应用配置示例中WS类反向代理示例。）

2、C=SS+v2ray-plugin+tls（tls由caddy提供及处理，不需配置。）

3、D=vless+h2c+tls（tls由caddy提供及处理，不需配置；另可添加或更改为其它h2c类应用，参考服务端单一应用配置示例中h2c类反向代理示例。）

4、G=vless+grpc+tls（tls由caddy提供及处理，不需配置；另可添加或更改为其它grpc类应用，参考服务端单一应用配置示例中grpc类反向代理示例。）
