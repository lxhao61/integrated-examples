...
8、nginx 支持 H2C server，但不支持 HTTP/1.1 server 与 H2C server 共用一个端口或一个进程（Unix Domain Socket 应用）；故回落分成 http/1.1 回落与 h2 回落，分别对应 nginx 的 HTTP/1.1 server 与 H2C server。
