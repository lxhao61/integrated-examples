...
3、nginx 不支持 h2c proxy，故 nginx 不能实现 Xray\v2ray 的 h2c 反向代理。
...
7、配置1：采用端口转发、端口回落\分流、端口分流，没有启用 PROXY protocol。配置2：采用进程转发、进程回落\分流、进程分流，没有启用 PROXY protocol。配置3：采用进程转发、进程回落\分流、进程分流，启用了 PROXY protocol。
