...
2、nginx 支持 h2c server，但不支持 http/1.1 server 与 h2c server 共用一个端口或一个进程（Unix Domain Socket 应用），故 vless-tcp-tls 或 trojan-tcp-tls 的端口回落或进程回落必须分开，分别对应 http/1.1 与 h2 回落。
...
6、此方法采用的是 SNI 方式实现共用 443 端口，支持 vless+tcp 与 trojan+tcp 完美共存，支持各自 xtls 应用，但需多个域名（多个证书或通配符证书）来标记分流。
