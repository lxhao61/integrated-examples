...
Xray 或 v2ray 前置（监听 443 端口），利用 vless+tcp+tls 或 vless+tcp+xtls 强大的回落/分流 WebSocket（WS） 特性及套娃 trojan+tcp 后回落 nginx，实现除 Xray 或 v2ray 的 KCP 应用外共用 443 端口。其应用如下：
