...
利用 Nginx 支持 SNI 分流特性，对 VLESS+Vision+TLS、Trojan+RAW+TLS、HTTP/2 server 进行 SNI 分流（四层转发），实现除 Xray 的 mKCP 应用外各应用共用 443 端口。其中 Nginx 同时为 VLESS+Vision+TLS 与 Trojan+RAW+TLS 提供 WEB 服务（回落应用），为 Xray 的 WebSocket、gRPC 提供反向代理，其应用如下：
