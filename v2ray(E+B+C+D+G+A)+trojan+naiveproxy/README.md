...
5、因 trojan-go 或 trojan 不支持 PROXY protocol，故 caddy SNI 分流 trojan-go 或 trojan 不启用此项应用；故共用 WEB 服务的 vless+tcp+xtls 或 vless+tcp+tls 回落也不启用此项应用，即全部回落不启用此项应用。
