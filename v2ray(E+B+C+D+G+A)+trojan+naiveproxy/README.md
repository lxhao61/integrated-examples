...
10、配置1：采用端口转发、端口回落\分流、端口分流，没有启用 PROXY protocol。配置2：采用进程转发、端口回落\分流、进程分流（对应trojan除外），没有启用 PROXY protocol。配置3：采用进程转发、端口回落\分流、进程分流（对应trojan除外），启用了 PROXY protocol（端口回落除外）。
