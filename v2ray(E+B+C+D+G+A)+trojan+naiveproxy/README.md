...
4、因 trojan-go 或 trojan 不支持 Unix Domain Socket 监听接收与回落转发，故 caddy SNI 分流 trojan-go 或 trojan 仅启用端口转发；故与 trojan-go 或 trojan 共用 WEB 服务的 vless+tcp+xtls 或 vless+tcp+tls 回落也仅端口回落，即全部端口回落。
