...
2、v2ray_DS_config.json 采用 Unix Domain Socket 应用组合实现，效率高，但在 Windows 10 Build 17036 前不可用。v2ray_redirect_config.json 采用 local loopback 应用（redirect）组合实现，效率稍低，但可适用全部服务器。
