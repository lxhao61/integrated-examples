...
1、nginx 支持 h2c server，但不支持 http/1.1 server 与 h2c server 共用一个端口或一个进程（Unix Domain Socket 应用）；故只要 vless+tcp+tls 以 h2 或 http/1.1 自适应协商连接，回落就必须按照 http/1.1 回落与 h2 回落分开，分别对应 nginx 的 http/1.1 server 与 h2c server。
