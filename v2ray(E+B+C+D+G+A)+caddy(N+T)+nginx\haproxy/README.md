...
利用 nginx 或 haproxy 支持 SNI 分流特性，对 vless+tcp+xtls 或 vless+tcp+tls、HTTPS server 进行 SNI 分流（四层转发），实现除 Xray 或 v2ray 的 KCP 应用外共用 443 端口。其中 vless+tcp+xtls 或 vless+tcp+tls 为 WebSocket（WS） 提供分流转发；caddy 为 vless+tcp+xtls 或 vless+tcp+tls 提供回落服务，为 Xray 或 v2ray 的 H2C 与 gRPC 进行反向代理，为 naiveproxy 与 trojan-go 提供正向代理。包括应用如下：
