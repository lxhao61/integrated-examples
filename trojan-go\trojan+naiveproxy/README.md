...
7、trojan-go 完全兼容原版 trojan，还有自己的特色：服务端同时支持原版 trojan 应用与自己的 Websocket 应用；支持 CDN 流量中转(基于 WebSocket over TLS)；支持使用 AEAD 对 trojan 协议流量进行二次加密(基于 Shadowsocks AEAD)。
