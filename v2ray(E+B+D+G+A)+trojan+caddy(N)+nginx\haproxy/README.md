...
15、配置1：使用 Local Loopback 连接，且启用了 PROXY protocol（全部回落除外）。配置2：使用 UDS 连接（对应 shadowsocks+grpc+tls、SNI 分流 trojan-go 或 trojan、全部回落除外。），且启用了 PROXY protocol（全部回落除外）。
