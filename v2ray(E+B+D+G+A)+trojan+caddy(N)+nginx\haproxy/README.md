...
15、配置1：使用 Local Loopback 连接，且启用了 PROXY protocol（全部回落除外）。配置2：使用 UDS 连接（对应 shadowsocks+grpc+tls、trojan-go/trojan 除外，使用 Local Loopback 连接。），且启用了 PROXY protocol（全部回落除外）。
