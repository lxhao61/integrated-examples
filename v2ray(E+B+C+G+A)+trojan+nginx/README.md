...
9、配置1：采用端口分流、端口回落\分流、端口转发。配置2：采用进程分流（对应trojan-go\trojan采用端口分流）、端口回落\分流（分流vless+WS采用进程分流）、进程转发。配置3：采用进程分流、端口回落\分流（分流vless+WS采用进程分流）、进程转发，且启用了 PROXY protocol（回落部分除外）。
