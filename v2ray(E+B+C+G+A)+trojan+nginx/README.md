...
4、因 trojan-go 或 trojan 不支持 Unix Domain Socket，故 SNI 分流 trojan-go 或 trojan 仅启用端口转发；故共用 WEB 服务的 vless+tcp+xtls 或 vless+tcp+tls 回落也仅端口回落，即全部端口回落。

5、因 trojan-go 或 trojan 不支持 PROXY protocol，故 nginx SNI 分流启用 PROXY protocol 须特殊处理（具体见配置示例）；故共用 WEB 服务的 vless+tcp+xtls 或 vless+tcp+tls 回落也不启用此项应用，即全部回落不启用此项应用。
