...
4、G=vless+grpc+tls（tls由nginx供及处理，不需配置；另可改成或添加其它gRPC类应用，参考对应服务端单一应用配置示例。）
...
8、配置1：采用端口分流、端口回落\分流、端口转发。配置2：采用进程分流（对应trojan-go\trojan采用端口分流）、端口回落\分流（分流vless+WS采用进程分流）、进程转发。配置3：采用进程分流、端口回落\分流（分流vless+WS采用进程分流）、进程转发（trojan-go\trojan采用端口转发），且启用了 PROXY protocol（trojan-go\trojan端口回落没有采用）。
