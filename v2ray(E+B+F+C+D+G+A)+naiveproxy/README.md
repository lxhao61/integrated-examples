...
2、caddy 版本不小于 v2.2.0-rc.1 版才支持 H2C proxy，即支持 Xray\v2ray 的 H2C（gRPC） 反向代理。
...
7、本示例采用的是 SNI 方式实现共用443端口，支持 Xray\v2ray（vless+tcp+tls）、Xray\v2ray（trojan+tcp+tls）、caddy（HTTPS server） 完美共存，支持各自特色应用，但需多个域名来标记分流。
