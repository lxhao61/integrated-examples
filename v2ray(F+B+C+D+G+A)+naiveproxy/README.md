...
7、naiveproxy（基于caddy的forwardproxy插件实现，TLS由trojan+tcp+tls/xtls提供及处理，不需配置。）
...
9、本示例 caddy 的 Caddyfile 格式配置与 json 格式配置二选一即可（除申请证书及密钥配置外，完全等效。）。若使用 caddy 申请证书及密钥推荐使用 json 格式配置，优化更好。
