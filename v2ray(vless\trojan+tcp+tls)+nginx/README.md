...
本示例配置包含 Xray（vless+tcp+xtls） 或 Xray\v2ray（vless+tcp+tls） 与 Xray（trojan+tcp+xtls） 或 Xray\v2ray（trojan+tcp+tls） 应用。Xray\v2ray 服务端前置（监听443端口）处理来自墙内的 HTTPS 请求，如果是合法的 Xray\v2ray 客户端请求，那么为该请求提供服务（科学上网）；否则将已解除 TLS 的流量请求回落（转发）给 WEB 服务器 nginx，由 nginx 为其提供服务。
...
7、因 v2ray 的 bug，trojan+tcp+tls 应用不支持 http/1.1 回落与 h2 回落分开；故若使用 Xray\v2ray（trojan+tcp+tls） 回落 nginx 应用，必须删除本示例中 http/1.1 连接参数、 http/1.1 回落配置及 h2 回落匹配参数（即保留 h2 连接与 h2 回落），或 h2 连接参数与回落配置（即保留 http/1.1 连接与 http/1.1 回落）。（即二选一）
